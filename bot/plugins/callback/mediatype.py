...
from bot.utils.media_type import add_media_type, get_media_type, remove_media_type
...
async def mediatype_toggle_view(bot: Client, query: CallbackQuery):
    ...
    for i, (media_type_value, display_name) in enumerate(
        settings.ALL_MEDIA_TYPES.items()
    ):
        ...
        if is_enabled:
            ...
            button_text = (
                f"❌ {display_name.split(' ')[-1]}"  # Show just the type name with X
            )
        ...
        if len(row) == 2 or i == len(settings.ALL_MEDIA_TYPES) - 1:
            buttons.append(row)
            row = []
...
async def mediatype_toggle_handler(bot: Client, query: CallbackQuery):
    ...
    if media_type:
        display_name = settings.ALL_MEDIA_TYPES.get(
            media_type_value, f"Type {media_type_value}"
        )
...
async def mediatype_enable_all(bot: Client, query: CallbackQuery):
    ...
    all_media_types = list(settings.ALL_MEDIA_TYPES.keys())

    # Set all media types as enabled
    await db.config.col.update_one(
        {"name": "media_type"}, {"$set": {"value": all_media_types}}, upsert=True
    )
...
async def mediatype_disable_all(bot: Client, query: CallbackQuery):
    ...
    # Set empty list
    await db.config.col.update_one(
        {"name": "media_type"}, {"$set": {"value": []}}, upsert=True
    )
...
async def mediatype_reset(bot: Client, query: CallbackQuery):
    ...
    await db.config.col.update_one(
        {"name": "media_type"}, {"$set": {"value": default_media_types}}, upsert=True
    )
